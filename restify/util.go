...
// Logger interface is to abstract the logging from Restify. Gives control to
// the Restify users, choice of the logger.
...
func (l *logger) Errorf(format string, v ...interface{}) {
	l.output("ERROR Restify "+format, v...)
}

func (l *logger) Warnf(format string, v ...interface{}) {
	l.output("WARN Restify "+format, v...)
}

func (l *logger) Debugf(format string, v ...interface{}) {
	l.output("DEBUG Restify "+format, v...)
}
...
// UnmarshalClient content into object from JSON or XML
func UnmarshalClient(c *Client, ct string, b []byte, d interface{}) (err error) {
	if IsJSONType(ct) {
...
// RequestLog struct is used to collected information from Restify request
// instance for debug logging. It sent to request log callback before Restify
...
// ResponseLog struct is used to collected information from Restify response
// instance for debug logging. It sent to response log callback before Restify
// actually logs the information.
...
func writeMultipartFormFile(w *multipart.Writer, fieldName, fileName string, r io.Reader) error {
	...
	size, err := r.Read(_buffer)
	if err != nil && err != io.EOF {
		return err
	}

	partWriter, err := w.CreatePart(createMultipartHeader(fieldName, fileName, http.DetectContentType(_buffer)))
	if err != nil {
		return err
	}

	if _, err = partWriter.Write(_buffer[:size]); err != nil {
		return err
	}
...
func composeHeaders(c *Client, r *Request, _header http.Header) string {
	str := make([]string, 0, len(_header))
	for _, k := range sortHeaderKeys(_header) {
		var v string
		if k == "Cookie" {
			cv := strings.TrimSpace(strings.Join(_header[k], ", "))
			...
			v = strings.TrimSpace(fmt.Sprintf("%25s: %s", k, strings.Join(_header[k], ", ")))
		}
		if v != "" {
			str = append(str, "\t"+v)
		}
...
func sortHeaderKeys(_header http.Header) []string {
	keys := make([]string, 0, len(_header))
	for key := range _header {
...
func copyHeaders(_header http.Header) http.Header {
	nh := http.Header{}
