...
type (
	// RedirectPolicy to regulate the redirects in the Restify client.
...
//
//	Restify.SetRedirectPolicy(NoRedirectPolicy())
...
//
//	Restify.SetRedirectPolicy(FlexibleRedirectPolicy(20))
...
// DomainCheckRedirectPolicy is convenient method to define domain name redirect rule in Restify client.
// Redirect is allowed for only mentioned host in the policy.
//
//	Restify.SetRedirectPolicy(DomainCheckRedirectPolicy("host1.com", "host2.org", "host3.net"))
...
// Restify will add all the headers during a redirect for the same host
