...
const debugRequestLogKey = "_restify_debug_request_audit_"
...
func parseRequestURL(c *Client, r *Request) error {
	...
	// Since not feasible in `SetQuery*` Restify methods, because
...
CL:
	// by default Restify won't set content length, you can if you want to :)
	...
	if IsJSONType(ct) || IsXMLType(ct) {
		...
		if res.IsSuccess() {
			...
			if res.Request.Result != nil {
				err = UnmarshalClient(c, ct, res.body, res.Request.Result)
				return
			}
		}
		...
		if res.IsError() {
			...
			if res.Request.Error != nil {
				err = UnmarshalClient(c, ct, res.body, res.Request.Error)
			}
		}
	}
...
func saveResponseIntoFile(c *Client, res *Response) error {
	if res.Request.isSaveResponse {
		...
		defer closeReq(outFile)

		// io.Copy reads maximum 32kb size, it is perfect for large file download too
		defer closeReq(res.RawResponse.Body)
...
func getBodyCopy(r *Request) (*bytes.Buffer, error) {
	...
	// Note: Restify user have to watchout for large body size of `io.Reader`
