...
var (
	...
	hdrUserAgentValue = "restify/" + Version + " (https://github.com/sivaosorg/govm)"
...
// Client struct is used to create Restify client with client level settings,
// these settings are applicable to all the request raised from the client.
//
// Restify also provides an options to override most of the client settings
// at request level.
type Client struct {
	BaseURL               string
...
//	client.SetHostURL("http://s.org.com")
//
//	// Setting HTTPS address
//	client.SetHostURL("https://s.org.com")
//
...
//
//	// Setting HTTP address
//	client.SetBaseURL("http://s.org.com")
//
//	// Setting HTTPS address
//	client.SetBaseURL("https://s.org.com")
//
// Since v2.7.0
...
// SetCookieJar method sets custom http.CookieJar in the Restify client. Its way to override default.
//
// For Example: sometimes we don't want to save cookies in api contacting, we can remove the default
// CookieJar in Restify client.
//
...
//	}
//
//	// Setting a cookies into Restify
...
//
// For Example: To set the header for username "go-Restify" and password "welcome"
//
//	client.SetBasicAuth("go-Restify", "welcome")
//
...
// Its gets applied after default Restify request middlewares and before request
// been sent from Restify to host server.
//
//	client.OnBeforeRequest(func(c *Restify.Client, r *Restify.Request) error {
...
// Once we receive response from host server, default Restify response middleware
// gets applied and then user assigened response middlewares applied.
//
//	client.OnAfterResponse(func(c *Restify.Client, r *Restify.Response) error {
...
//
//	client.OnError(func(req *Restify.Request, err error) {
//		if v, ok := err.(*Restify.ResponseError); ok {
...
// SetPreRequestHook method sets the given pre-request function into Restify client.
...
// SetDebug method enables the debug mode on Restify client. Client logs details of every request and response.
...
// OnRequestLog method used to set request log callback into Restify. Registered callback gets
// called before the Restify actually logs the information.
...
// OnResponseLog method used to set response log callback into Restify. Registered callback gets
// called before the Restify actually logs the information.
...
// SetDisableWarn method disables the warning message on Restify client.
//
// For Example: Restify warns the user when BasicAuth used on non-TLS mode.
//
...
// SetAllowGetMethodPayload method allows the GET method with payload on Restify client.
//
// For Example: Restify allows the user sends request with a payload on HTTP GET method.
//
...
// SetLogger method sets given writer for logging Restify request and response details.
//
// Compliant to interface `Restify.Logger`.
...
// By default Restify won't set `Content-Length`.
//
...
// SetError method is to register the global or client common `Error` object into Restify.
...
// SetRedirectPolicy method sets the client redirect poilicy. Restify provides ready to use
...
func (c *Client) SetRedirectPolicy(policies ...interface{}) *Client {
	for _, p := range policies {
		if _, ok := p.(RedirectPolicy); !ok {
			c.log.Errorf("%v does not implement Restify.RedirectPolicy (missing Apply method)",
				functionName(p))
		}
	}
...
// SetRetryCount method enables retry on Restify client and allows you
// to set no. of retry count. Restify uses a Backoff mechanism.
...
// By default, Restify uses `encoding/json` package to marshal the request body.
...
// By default, Restify uses `encoding/json` package to unmarshal the response body.
...
// By default, Restify uses `encoding/xml` package to marshal the request body.
...
// By default, Restify uses `encoding/xml` package to unmarshal the response body.
...
// SetRetryResetReaders method enables the Restify client to seek the start of all
...
// SetProxy method sets the Proxy URL and Port for Restify client.
//
...
// RemoveProxy method removes the proxy configuration from Restify client
//
//	client.RemoveProxy()
...
// SetCertificates method helps to set client certificates into Restify conveniently.
...
// SetRootCertificate method helps to add one or more root certificates into Restify client
//
...
// SetRootCertificateFromString method helps to add one or more root certificates into Restify client
//
...
// If the output directory not exists then Restify creates one. This setting is optional one,
...
// compatible interface implementation in the Restify client.
//
// Note:
//
// - If transport is not type of `*http.Transport` then you may not be able to
// take advantage of some of the Restify client settings.
//
// - It overwrites the Restify client transport instance and it's configurations.
//
...
// SetScheme method sets custom scheme in the Restify client. It's way to override default.
//
...
// SetDoNotParseResponse method instructs `Restify` not to parse the response body automatically.
// Restify exposes the raw response body as `io.ReadCloser`. Also do not forget to close the body,
...
// taken over the control of response parsing from `Restify`.
...
// EnableTrace method enables the Restify client trace for the requests fired from
// the client using `httptrace.ClientTrace` and provides insights.
//
//	client := Restify.New().EnableTrace()
//
...
// DisableTrace method disables the Restify client trace. Refer to `Client.EnableTrace`.
//
// Since v2.0.0
...
// IsProxySet method returns the true is proxy is set from Restify client otherwise
// false. By default proxy is set from environment, refer to `http.ProxyFromEnvironment`.
func (c *Client) IsProxySet() bool {
	return c.proxyURL != nil
}

// GetClient method returns the current `http.Client` used by the Restify client.
...
func (c *Client) execute(req *Request) (*Response, error) {
	...
	// to modify the *Restify.Request object
